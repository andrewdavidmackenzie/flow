...
impl Implementation for Stdout {
    fn run(&self, inputs: &[Value]) -> (Option<Value>, RunAgain) {
        ...
        if let Ok(mut client) = self.client.lock() {
            match input {
                Value::Null => client.send_event(Event::StdoutEOF),
                Value::String(string) => client.send_event(Event::Stdout(string.to_string())),
                Value::Bool(boolean) => client.send_event(Event::Stdout(boolean.to_string())),
                Value::Number(number) => client.send_event(Event::Stdout(number.to_string())),
