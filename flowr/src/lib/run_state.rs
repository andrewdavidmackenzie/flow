...
/// Running Waiting   `Output`: it has one input or more empty, so it can't run     `running_to_waiting_on_done`
///
...
pub struct RunState {
    ...
    /// Sub-flows that went idle and need flow initializers run on next restart
    flows_needing_init: HashSet<usize>,
    /// Sub-flows that are currently executing an iteration
    flows_in_progress: HashSet<usize>,
}
...
impl RunState {
    ...
    pub fn new(submission: Submission) -> Self {
        RunState {
            ...
            flows_needing_init: HashSet::<usize>::new(),
            flows_in_progress: HashSet::<usize>::new(),
        }
    }
    ...
    pub(crate) fn init(&mut self) -> Result<()> {
        ...
        // Build flow hierarchy if not already populated by the compiler
        self.submission.manifest.build_flow_hierarchy();
    ...
    // Check if the flow has gone idle and run flow initializers if so
    #[allow(unused_variables, unused_assignments, unused_mut)]
    fn unblock_flows(
        &mut self,
        job: &Job,
        ...
        // A flow is idle only if it has no busy functions AND none of its
        // child sub-flows are busy (recursive check)
        if self.is_flow_idle(job.flow_id) {
            ...
            // For sub-flows: clear stale internal values
            if job.flow_id != 0 {
    ...
    fn is_flow_idle(&self, flow_id: usize) -> bool {
        ...
        // Recursively check child sub-flows
        for flow_info in self.submission.manifest.flows() {
...
mod test {
    ...
    mod functional_tests {
        ...
        fn next_works() {
            ...
            state.create_jobs(0, 0).expect("Could not make ready");
