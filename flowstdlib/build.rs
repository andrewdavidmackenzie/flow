...
fn main() -> io::Result<()> {
    ...
    let out_dir = if lib_root_dir.file_name().expect("Could not get file_name")
        == env!("CARGO_PKG_NAME") {
        // local build from source folder - always compile to same folder "target/flowstdlib"
        let workspace_root_dir = lib_root_dir.parent()
    ...
    // If the "wasm" feature is activated, then don't set "-n" and flowc will compile
    // implementations to wasm.
    #[cfg(feature = "wasm")]
