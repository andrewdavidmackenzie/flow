...
#[flow_function]
fn _sort(inputs: &[Value]) -> (Option<Value>, RunAgain) {
    if inputs[0].is_null() {
...
mod test {
    ...
    #[test]
    fn sort_invalid() {
        let (result, _) = _sort(&[json!("Hello World")]);
        assert_eq!(result, None);
    }

    #[test]
    fn sort_one() {
        let (result, _) = _sort(&[json!([1])]);
    ...
    #[test]
    fn sort_array() {
        let (result, _) = _sort(&[json!([7, 1, 4, 8, 3, 9])]);
    ...
    fn sort_array_repeats() {
        let (result, _) = _sort(&[json!([7, 1, 8, 4, 8, 3, 1, 9])]);
