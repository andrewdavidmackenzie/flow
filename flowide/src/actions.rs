...
fn load_libs(loader: &mut Loader, provider: &dyn Provider, flowruntime_manifest: LibraryManifest) -> Result<String, String> {
    ...
    loader.add_lib(provider, flowruntime_manifest, &flowruntime_manifest.metadata.name).map_err(|e| e.to_string())?;
    ...
    Ok("Added the 'flowruntime' and 'flowstdlibs'".to_string())
}
