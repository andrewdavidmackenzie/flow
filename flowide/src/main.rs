...
impl Application for FlowIde {
    ...
    fn update(&mut self, message: Message) -> Command<Message> {
        match message {
            Message::Start => {
//                self.client_connection.send(ClientMessage::ClientSubmission(submission))?;
            }
        }
    ...
    fn view(&self) -> Element<Message> {
        row![
            text(self.filename.clone()).size(50),
            text(self.args.clone()).size(50),
...
/// Run `flowide`. After setting up logging and parsing the command line arguments invoke `flowrlib`
/// and return any errors found.
fn run() -> crate::errors::Result<()> {
...
fn client_and_coordinator(
    ...
    let (client, override_args) = client(
        debug_this_flow,
        discovery_port,
    )?;

    submit(client, runtime_client_connection, override_args, matches, lib_search_path, debug_this_flow)
}

fn submit(mut client: CliRuntimeClient,
    ...
    client.set_args(flow_args, override_args);

    client.set_display_metrics(matches.get_flag("metrics"));
    ...
    Ok(client.event_loop(client_connection)?)
}
...
fn client(
    debug_this_flow: bool,
    discovery_post: u16,
) -> Result<(CliRuntimeClient, Arc<Mutex<Vec<String>>>)> {
