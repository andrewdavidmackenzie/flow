...
use crate::file_ops;
use crate::flow_canvas::{CanvasAction, CanvasMessage};
use crate::hierarchy_panel::{FlowHierarchy, HierarchyMessage};
use crate::library_mgmt;
use crate::library_panel::{LibraryAction, LibraryMessage, LibraryTree};
use crate::utils;
use crate::window_state::{FunctionViewer, WindowKind, WindowState};
use crate::{load_initial_flow, parse_cli_args, setup_lib_search_path};
...
#[derive(Debug, Clone)]
pub(crate) enum FunctionEditMessage {
...
pub(crate) struct FlowEdit {
    pub(crate) windows: HashMap<window::Id, WindowState>,
    pub(crate) root_flow: FlowDefinition,
    pub(crate) root_window: Option<window::Id>,
    pub(crate) focused_window: Option<window::Id>,
    pub(crate) library_tree: LibraryTree,
    pub(crate) show_lib_paths: bool,
...
pub(crate) fn toolbar_btn(_theme: &Theme, status: button::Status) -> button::Style {
...
pub(crate) fn toolbar_btn_active(_theme: &Theme, status: button::Status) -> button::Style {
    let is_hovered = matches!(status, button::Status::Hovered);
    button::Style {
...
impl FlowEdit {
    ...
    pub(crate) fn unsaved_edit_count(&self) -> usize {
        self.windows
            .values()
