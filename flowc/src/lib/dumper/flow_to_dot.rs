...
/// }
/// ```
pub fn dump_flow(flow: &FlowDefinition, output_dir: &Path, provider: &dyn Provider) -> Result<()> {
    info!(
...
fn inner_dump_flow(
    ...
    for subprocess in &flow.subprocesses {
        if let FlowProcess(ref subflow) = subprocess.1 {
            inner_dump_flow(subflow, level + 1, target_dir, provider)?;
        }
    }
...
fn write_flow_to_dot(flow: &FlowDefinition, dot_file: &mut dyn Write) -> Result<()> {
...
fn process_references_to_dot(flow: &FlowDefinition) -> Result<String> {
    ...
    for process_ref in &flow.process_refs {
        ...
        match process {
            FlowProcess(ref subflow) => contents.push_str(&subflow_to_dot(
                subflow,
                file_path.as_path(),
                subflow.route(),
            )?),
...
fn subfunction_to_dot(function: &FunctionDefinition, parent: &Path) -> Result<String> {
    ...
    let function_source_path = function
        .get_source_url()
        .to_file_path()
    ...
    dot_string.push_str(&input_initializers_to_dot(
        function,
        function.route().as_ref(),
    )?);

    Ok(dot_string)
}

pub(crate) fn input_initializers_to_dot(
...
pub(crate) fn output_name_to_port<T: Hash>(t: &T) -> Result<&str> {
    Ok(OUTPUT_PORTS
        .get(index_from_name(t, OUTPUT_PORTS.len()))
...
fn absolute_to_relative(target: &Path, source: &Path) -> Result<String> {
    let mut current_path = source
        .parent()
        .ok_or("Could not get directory containing source")?
        .to_path_buf();
    ...
    let sub_path_from_common_point = target
        .strip_prefix(current_path.as_path())
        .map_err(|_| "Could not calculate sub-path")?;
    relative_path_to_root =
        relative_path_to_root.add(&sub_path_from_common_point.to_string_lossy());
...
mod test {
    ...
    fn sub_dir_relative_path() {
        ...
        let relative =
            absolute_to_relative(target, parent).expect("Could not form a relative path");
    ...
    fn sub_dir_mixed_schemes_relative_path() {
        let target_url = Url::parse(
            "file:///Users/andrew/workspace/flow/flowr/examples/mandlebrot/escapes/escapes.html",
        )
        .expect("Could not parse Url");
        let target = target_url
            .to_file_path()
            .expect("Could not convert to file path");
        let parent =
            Path::new("/Users/andrew/workspace/flow/flowr/examples/mandlebrot/render.dot.svg");
    ...
    fn other_branch_relative_path() {
        ...
        let parent = Path::new(
            "file:///Users/andrew/workspace/flow/flowr/examples/mandlebrot/render.dot.svg",
        );
        ...
        assert_eq!(
            relative,
            "../../../../../.flow/lib/flowstdlib/control/index_f.html"
        );
    }
}
