...
impl IO {
    ...
    /// # Errors
    ///
    /// Will return `Err` if an initializer has already been set for this `IO` or if the type
    /// of the initializer is not compatible with the type of the `IO`
    ...
    pub fn set_initializer(
        ...
        match self.initializer {
            Some(_) => bail!(
                "Attempt to set two InputInitializers on IO @ {}",
                self.route
            ),
            None => {
                if let Some(initializer) = &function_initializer {
                    DataType::compatible_types(
                        &[DataType::value_type(initializer.get_value())?],
                        self.datatypes(),
                        &Route::default(),
                    )
...
pub trait Find {
    ...
    /// # Errors
    ///
    /// Will return `Err` if a route cannot be found
...
impl Find for IOSet {
    ...
    fn find_by_subroute(&mut self, sub_route: &Route) -> Result<IO> {
        for io in &mut *self {
            for datatype in io.datatypes().clone() {
                if datatype.is_array() || datatype.is_generic() {
                    let (array_route, index, array_index) =
                        sub_route.without_trailing_array_index();
        ...
        bail!(
            "No IO with sub-route '{sub_route}' was found.\n\
...
mod test {
    ...
    fn deserialize_empty_string() {
        ...
        assert_eq!(
            ...
            Some(&DataType::from(GENERIC_TYPE))
        );
