...
pub fn datatype_or_datatype_array<'de, D>(
    deserializer: D,
) -> std::result::Result<Vec<DataType>, D::Error>
where
    D: Deserializer<'de>,
{
...
impl DataType {
    ...
    pub fn array_type(&self) -> Option<DataType> {
        if self.is_generic() {
            ...
            self.string
                .strip_prefix(&format!("{ARRAY_TYPE}/"))
                .map(DataType::from)
        } else {
            None
        }
    ...
    pub fn compatible_types(
        ...
        for from_type in from {
            ...
            if !compatible_destination_type {
                bail!(
                    "Could not find a compatible destination type in '{:?}' for '{:?}'",
                    from,
                    to
                )
            }
    ...
    fn subtype_using_subroute(full_type: &DataType, subroute: &Route) -> Result<DataType> {
        ...
        if subroute.depth() > full_type_split.len() {
            bail!(
                "Depth of subroute '{}' is greater than the depth of the type '{}'",
                subroute,
                full_type
            )
        }
        ...
        Ok(DataType::from(
            full_type_split.split_off(subroute.depth() - 1).join("/"),
        ))
    }
    ...
    fn two_compatible_types(from: &DataType, to: &DataType) -> Result<()> {
        ...
        match from.type_array_order() - to.type_array_order() {
            ...
            1 => {
                return Self::two_compatible_types(
                    &from.array_type().ok_or("From type is not an array!")?,
                    to,
                )
            }
            ...
            _ => bail!(
                "Cannot encapsulate/serialize arrays with a order difference of more than two"
            ),
        }
...
mod test {
    ...
    fn subtype_empty_route() {
        ...
        assert_eq!(
            DataType::subtype_using_subroute(&array_of_numbers_type, &Route::from(""))
                .expect("Could not get subtype"),
            array_of_numbers_type
        );
    }
    ...
    fn invalid_subtype_route() {
        assert!(
            DataType::subtype_using_subroute(&DataType::from(NUMBER_TYPE), &Route::from("/1"))
                .is_err()
        );
    }
    ...
    fn subtype_of_generic() {
        assert!(DataType::subtype_using_subroute(
            &DataType::from(GENERIC_TYPE),
            &Route::from("/1")
        )
        .is_ok());
    }
    ...
    fn valid_data_json_type() {
        ...
        json_type
            .valid()
            .expect("'object' DataType should be valid");
    }
    ...
    mod type_conversion {
        ...
        use crate::model::datatype::{
            DataType, ARRAY_TYPE, BOOLEAN_TYPE, GENERIC_TYPE, NULL_TYPE, NUMBER_TYPE, OBJECT_TYPE,
            STRING_TYPE,
        };
        ...
        fn valid_type_conversions() {
            let valid_type_conversions: Vec<(String, String, &str)> = vec![
                ...
                (
                    format!("{ARRAY_TYPE}/{NUMBER_TYPE}"),
                    GENERIC_TYPE.into(),
                    "",
                ),
                (
                    format!("{ARRAY_TYPE}/{ARRAY_TYPE}/{NUMBER_TYPE}"),
                    GENERIC_TYPE.into(),
                    "",
                ),
                ...
                (
                    STRING_TYPE.into(),
                    format!("{ARRAY_TYPE}/{STRING_TYPE}"),
                    "",
                ),
                (
                    BOOLEAN_TYPE.into(),
                    format!("{ARRAY_TYPE}/{BOOLEAN_TYPE}"),
                    "",
                ),
                ...
                (
                    NUMBER_TYPE.into(),
                    format!("{ARRAY_TYPE}/{GENERIC_TYPE}"),
                    "",
                ),
                ...
                (
                    format!("{ARRAY_TYPE}/{ARRAY_TYPE}/{NULL_TYPE}"),
                    format!("{ARRAY_TYPE}/{ARRAY_TYPE}/{NULL_TYPE}"),
                    "",
                ),
                (
                    format!("{ARRAY_TYPE}/{ARRAY_TYPE}/{STRING_TYPE}"),
                    format!("{ARRAY_TYPE}/{ARRAY_TYPE}/{STRING_TYPE}"),
                    "",
                ),
                (
                    format!("{ARRAY_TYPE}/{ARRAY_TYPE}/{BOOLEAN_TYPE}"),
                    format!("{ARRAY_TYPE}/{ARRAY_TYPE}/{BOOLEAN_TYPE}"),
                    "",
                ),
                (
                    format!("{ARRAY_TYPE}/{ARRAY_TYPE}/{OBJECT_TYPE}"),
                    format!("{ARRAY_TYPE}/{ARRAY_TYPE}/{OBJECT_TYPE}"),
                    "",
                ),
                ...
                (
                    format!("{ARRAY_TYPE}/{ARRAY_TYPE}/{GENERIC_TYPE}"),
                    format!("{ARRAY_TYPE}/{NUMBER_TYPE}"),
                    "",
                ),
                (
                ...
                (
                    format!("{OBJECT_TYPE}/{NUMBER_TYPE}"),
                    NUMBER_TYPE.into(),
                    "/name",
                ),
            ];
        ...
        fn invalid_type_conversions() {
            let invalid_type_conversions: Vec<(String, String, &str)> = vec![
                ...
                (
                    format!("{OBJECT_TYPE}/{STRING_TYPE}"),
                    STRING_TYPE.into(),
                    "",
                ),
            ];
        ...
        fn multiple_output_type_to_superset_input_types() {
            let from_io = IO::new(vec![STRING_TYPE.into(), NUMBER_TYPE.into()], "/p1/output");
            let to_io = IO::new(
                vec![STRING_TYPE.into(), NUMBER_TYPE.into(), ARRAY_TYPE.into()],
                "/p2",
            );
            assert!(DataType::compatible_types(
                from_io.datatypes(),
